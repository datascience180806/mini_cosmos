...
class Cosmos3Config:
    """
    Cấu hình thông số kỹ thuật cho Version 8 (QK-Norm + LayerScale Architecture ~4.03B Params)
    Đồng quy mô với Version 5 để so sánh trực tiếp ở mốc 4B:
    - hidden_dim: 3072
...
class QKNormGroupedQueryAttention(nn.Module):
    """
    Attention Layer nâng cấp với QK-Norm (RMSNorm trên Query và Key) + GQA & RoPE cho Version 8.
    Cơ chế QK-Norm giúp triệt tiêu hiện tượng Attention Score Explosion trong chuẩn FP16.
    """
...
class Cosmos3ToyModel(nn.Module):
    """
    Mô hình Cosmos 3 Version 8 (QK-Norm & LayerScale Architecture ~4.03B Params):
    - Đồng quy mô 4.03B với Version 5 để so sánh trực tiếp.
    - QK-Norm (RMSNorm trên Q & K) giúp ổn định ma trận chú ý FP16.
    - LayerScale Residual Connection nâng cao chất lượng lan truyền tín hiệu.
    """
    def __init__(self, config: Cosmos3Config):
        super().__init__()
        self.config = config
        ...
        self.blocks = nn.ModuleList([Cosmos3BlockV8(config) for _ in range(config.num_layers)])
